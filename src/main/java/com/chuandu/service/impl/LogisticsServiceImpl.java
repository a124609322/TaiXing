                ...
                switch (j) {
                    case Constant.IMPORT_ROBOT_CODE:
                        ...
                        if (StringUtils.isNullOrEmpty(robot.getBatchcode())) {
                            result.put("success", false);
                            result.put("msg", "Excel第" + i + "行" + "单号/批次号为空，请重新上传~！");
                    ...
                    case Constant.IMPORT_ROBOT_INTERVAL:
                        String intervalReg = CommonUtil.getPropertiesValue("config.properties","intervalRegex");
                        if(cell.matches(intervalReg)){
                            robot.setInterval(cell);
                        }else{
                            result.put("success", false);
                            result.put("msg", "Excel第" + i + "行" + "时间间隔格式不对，请重新上传~！");
                            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
                            return result;
                        }
                        break;
